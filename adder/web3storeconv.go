...
func (adder *Adder) Add(filename string, file fs.File, dirname string, fsys fs.FS) (ipld.Node, error) {
	if fsys == nil {
		fsys = &w3fs.OsFs{}
	}
	...
	nd, err := adder.addAll(filename, file, fi, dirname, fsys)
	if err != nil {
		return nil, err
	}
	return nd, nil
...
func (adder *Adder) addAll(filename string, f fs.File, fi fs.FileInfo, dirname string, fsys fs.FS) (ipld.Node, error) {
	if len(filename) < 1 {
		filename = fi.Name()
	}
	if err := adder.addFileOrDir(filename, f, fi, dirname, fsys, true); err != nil {
		return nil, err
	}
...
func (adder *Adder) addFile(path string, f fs.File) error {
	...
	// println("adding file", dagnode.Cid().String())
	// patch it into the root
